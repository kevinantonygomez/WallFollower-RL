...
class Q:
    ...
    def update_q_table(self, prev_state, curr_state, action, alpha=0.2, gamma=0.8):
        old_val = self.Q_table[prev_state][action]
        reward = self.get_reward(prev_state)
        opt_future_val = max(self.Q_table[curr_state])
        self.Q_table[prev_state][action] = old_val + (alpha * (reward + (gamma * opt_future_val) - old_val))
    ...
    def train(self, triton, gazebo):
        ...
        print("Entered Q_TD training mode!")
        while True:
        ...
        for n in range(1, self.EPISODES+1):
            ...
            for i in range(self.TIME_STEPS):
                ...
                self.update_q_table(curr_state, actual_curr_state, action) # curr_state here is now the previous state
                ...
                with open("q_td_trained_q_table.pkl", "wb+") as f:
